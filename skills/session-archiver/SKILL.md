...
2. For installation or debugging, inspect the repo-root `.codex/hooks.json`, the root `hooks/session_archiver_hook.py`, and the user's `~/.codex/config.toml`.
...
Then install or symlink the repo's `.codex/hooks.json` to one of Codex's discovered locations:

- `~/.codex/hooks.json` for home-global archiving
- `<target-repo>/.codex/hooks.json` for repo-local archiving

The shipped hook file defaults to `CODEX_SESSION_ARCHIVER_ROOT=/app/my-codex-skill`. If the repo lives elsewhere, set `CODEX_SESSION_ARCHIVER_ROOT` in the shell environment before starting Codex.
...
- If no archive file appears, verify `codex_hooks = true` and that the active `hooks.json` is in `~/.codex/` or `<repo>/.codex/`.
